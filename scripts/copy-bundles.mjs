 ...
 *
 * Payload dirs (`pi-extensions/`, `skills/`, `prompts/`) are raw `.ts`/`.md`
 * that pi loads via its own loader — they are excluded from `tsc` (see
 ...
 * NOT copied (tsc compiles it to `index.js`).
 *
...
const PAYLOAD_DIRS = ["pi-extensions", "skills", "prompts"];
