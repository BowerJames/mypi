 ...
 *
 * Always prefixes with `btw › `, collapses to `(no output)` when empty, and
 * caps length so a verbose answer does not flood the chat scrollback.
 */
export function formatResult(text: string): string {
	const trimmed = text.trim();
	if (!trimmed) return "btw › (no output)";
	return capResult(`btw › ${trimmed}`);
}
